...
import { LayoutDashboard, Users, LogOut, CheckCircle, Clock, Trash2, Power, Settings, Radio, Filter, Search, X, Download, MessageCircle, PlusCircle, IndianRupee, Edit, Building, MapPin, FileText, Upload } from 'lucide-react';
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    ...
    return () => { unsubSettings(); unsubBookings.forEach(unsub => unsub()); unsubCamps(); };
  }, [isAuthenticated]);
  ...
  const handleLogin = (e) => {
    ...
    if (password === 'Ankush@7987') { 
      ...
      setError('Incorrect Password! Please enter valid credentials.');
    }
  };
  ...
  const markAsCompleted = async (id, colName) => {
    ...
    if (appNumber !== null) {
      try {
        await updateDoc(doc(db, colName, id), { status: 'Completed', applicationNumber: appNumber || 'N/A' });
      } catch (err) {
        console.error("Error:", err); alert("Failed to update booking status!");
      }
    }
  };
  ...
  const submitPayment = async (e) => {
    ...
    try {
      ...
      console.error("Error:", err); alert("Failed to save payment details.");
  ...
  // --- NAYA FIX: WALK-IN TO UPLOAD FLOW ---
  const submitWalkIn = async (e) => {
    ...
    try {
      ...
      // 2. Save student details before resetting the form
      ...
      // 3. Prompt admin to upload documents immediately
      if(window.confirm(`Successfully Added: ${savedStudentDetails.fullName}\n\nDo you want to upload their documents now to save time?`)) {
        ...
        setTimeout(() => setIsUploadModalOpen(true), 300); // Slight delay for smooth animation
      }

    } catch (err) {
      console.error("Error:", err); alert("Failed to add walk-in student.");
    } finally { setSavingWalkIn(false); }
  };

  const openDocsModal = (docs, name) => {
  ...
  const exportToExcel = () => {
    ...
    const titleRow = `"${reportHeading}"`; 
    const emptyRow = `""`; 

    const headers = ["Token No.", "Student Name", "Mobile", "Category", "Exam", "Institute", "Batch", "Slot Date", "Slot Time", "Form Status", "Payment Status", "Payment Amount", "Payment Method", "Application No.", "Applied On"];
    
    const rows = filteredBookings.map(b => [
      b.tokenNumber || 'N/A', b.fullName || 'N/A', b.mobile || 'N/A', b.category || 'N/A', b.exam || 'N/A', b.institute || 'N/A', b.batchName || 'N/A', b.slotDate || 'N/A', b.slotTime || 'N/A', b.status || 'Pending', b.paymentStatus || 'Due', b.paymentAmount ? `₹${b.paymentAmount}` : 'N/A', b.paymentMethod || 'N/A', b.applicationNumber || 'N/A', formatTime(b.timestamp)
    ]);
  ...
  return (
    <div className="h-screen bg-gray-50 flex overflow-hidden">
      ...
      <WalkInModal 
        ...
        handleWalkInChange={handleWalkInChange} 
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-gray-900/80 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white rounded-2xl p-6 w-full max-w-2xl shadow-2xl relative my-8">
            ...
            <h2 className="text-xl font-bold text-gray-800 mb-6 border-b border-gray-100 pb-4 flex items-center gap-2">
              <Upload size={22} className="text-indigo-500"/> Upload Docs: {uploadTarget.fullName}
            </h2>
            
            <DocumentUploader
              studentId={uploadTarget.id}
              collectionName={uploadTarget.collectionName}
              studentName={uploadTarget.fullName}
              category={uploadTarget.category}
              onComplete={() => {
                alert("Documents uploaded and linked successfully!");
      ...
      {/* DOCUMENTS VIEWER MODAL UI WITH DOWNLOAD BUTTONS */}
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <div className="space-y-3">
              ...
              {selectedDocs.signatureUrl && (
                <div className="flex items-center justify-between p-3 bg-blue-50/50 border border-blue-100 rounded-xl">
                  <span className="font-bold text-blue-800 text-sm">✍️ Signature</span>
                  <div className="flex gap-2">
                    <a href={selectedDocs.signatureUrl} target="_blank" rel="noreferrer" className="px-3 py-1.5 bg-white text-blue-600 hover:bg-blue-600 hover:text-white border border-blue-200 rounded-lg text-xs font-bold transition-all">View</a>
                    <a href={getDownloadUrl(selectedDocs.signatureUrl)} download className="px-3 py-1.5 bg-emerald-500 text-white hover:bg-emerald-600 rounded-lg text-xs font-bold shadow-sm transition-all">Download</a>
                  </div>
                </div>
              )}
              {selectedDocs.tenthUrl && (
                <div className="flex items-center justify-between p-3 bg-indigo-50/50 border border-indigo-100 rounded-xl">
                  <span className="font-bold text-indigo-800 text-sm">📄 10th Marksheet</span>
                  <div className="flex gap-2">
                    <a href={selectedDocs.tenthUrl} target="_blank" rel="noreferrer" className="px-3 py-1.5 bg-white text-indigo-600 hover:bg-indigo-600 hover:text-white border border-indigo-200 rounded-lg text-xs font-bold transition-all">View</a>
                    <a href={getDownloadUrl(selectedDocs.tenthUrl)} download className="px-3 py-1.5 bg-emerald-500 text-white hover:bg-emerald-600 rounded-lg text-xs font-bold shadow-sm transition-all">Download</a>
                  </div>
                </div>
              )}
              {selectedDocs.domicileUrl && (
                <div className="flex items-center justify-between p-3 bg-indigo-50/50 border border-indigo-100 rounded-xl">
                  <span className="font-bold text-indigo-800 text-sm">📄 Niwash Praman</span>
                  <div className="flex gap-2">
                    <a href={selectedDocs.domicileUrl} target="_blank" rel="noreferrer" className="px-3 py-1.5 bg-white text-indigo-600 hover:bg-indigo-600 hover:text-white border border-indigo-200 rounded-lg text-xs font-bold transition-all">View</a>
                    <a href={getDownloadUrl(selectedDocs.domicileUrl)} download className="px-3 py-1.5 bg-emerald-500 text-white hover:bg-emerald-600 rounded-lg text-xs font-bold shadow-sm transition-all">Download</a>
                  </div>
                </div>
              )}
              {selectedDocs.casteUrl && (
                <div className="flex items-center justify-between p-3 bg-indigo-50/50 border border-indigo-100 rounded-xl">
                  <span className="font-bold text-indigo-800 text-sm">📄 Caste Cert.</span>
                  <div className="flex gap-2">
                    <a href={selectedDocs.casteUrl} target="_blank" rel="noreferrer" className="px-3 py-1.5 bg-white text-indigo-600 hover:bg-indigo-600 hover:text-white border border-indigo-200 rounded-lg text-xs font-bold transition-all">View</a>
                    <a href={getDownloadUrl(selectedDocs.casteUrl)} download className="px-3 py-1.5 bg-emerald-500 text-white hover:bg-emerald-600 rounded-lg text-xs font-bold shadow-sm transition-all">Download</a>
                  </div>
                </div>
              )}
            </div>
      ...
      <aside className="w-64 bg-gray-900 text-white hidden md:flex flex-col flex-shrink-0 z-20">
        ...
        <nav className="flex-1 p-4 space-y-3 mt-4">
          ...
          <button onClick={() => setActiveTab('camps')} className={`w-full flex items-center justify-between px-4 py-3 rounded-xl font-medium transition-all ${activeTab === 'camps' ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20' : 'text-gray-400 hover:text-white hover:bg-gray-800'}`}>
            ...
            {campRequests.filter(c => c.status === 'New Request').length > 0 && (
              <span className="bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">{campRequests.filter(c => c.status === 'New Request').length} New</span>
            )}
          </button>
      ...
      <main className="flex-1 p-6 lg:p-10 overflow-y-auto">
        ...
        {activeTab === 'dashboard' && (
          <div className="animate-in fade-in duration-500">
            <header className="flex flex-col xl:flex-row justify-between xl:items-center gap-4 mb-8">
              <div>
                <h1 className="text-3xl font-extrabold text-gray-900">Database Overview</h1>
                <p className="text-gray-500 mt-1">Real-time student booking records</p>
              </div>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {loading ? (
                      ...
                      <tr><td colSpan="5" className="p-8 text-center text-gray-500">No records found for the selected filters.</td></tr>
                    ) : (
                      filteredBookings.map((booking) => (
                        <tr key={booking.id} className="hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-0">
                          ...
                          <td className="p-4 align-top">
                            ...
                            <p className="text-[10px] text-gray-400 border-t border-gray-100 pt-1 mt-2">Recorded: {formatTime(booking.timestamp)}</p>
                          </td>
                          <td className="p-4 align-top">
                            <p className="font-black text-indigo-600 text-sm mb-2" title="System Token">{booking.tokenNumber}</p>
                            <div className="flex gap-2 items-center mb-2">
                              ...
                              {booking.paymentStatus === 'Paid' ? (
                                <div className="flex items-center gap-1">
                                  <span className="px-2 py-1 rounded text-[11px] font-bold bg-green-50 text-green-700 border border-green-200 flex items-center gap-1"><CheckCircle size={10}/> ₹{booking.paymentAmount} ({booking.paymentMethod})</span>
                            ...
                            {booking.applicationNumber && booking.applicationNumber !== 'N/A' && (
                              <p className="text-[11px] font-bold text-gray-600 bg-gray-200 px-2 py-1 rounded inline-block">App No: {booking.applicationNumber}</p>
                            )}
                          </td>
                          <td className="p-4 align-top">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              {booking.documents ? (
                                <button 
                                  onClick={() => openDocsModal(booking.documents, booking.fullName)}
                                  className="flex items-center justify-center p-2 bg-blue-50 text-blue-600 hover:bg-blue-500 hover:text-white rounded-lg transition-colors border border-blue-100" 
                                ...
                                <button 
                                  onClick={() => { setUploadTarget(booking); setIsUploadModalOpen(true); }} 
                                  className="flex items-center gap-1 px-3 py-2 text-xs font-bold text-blue-600 bg-blue-50 hover:bg-blue-600 hover:text-white border border-blue-200 rounded-lg transition-all shadow-sm" 
                              ...
                              <a href={`https://wa.me/91${booking.mobile}?text=Hello ${booking.fullName}, reaching out from EduFill regarding your application form...`} target="_blank" rel="noreferrer" className="flex items-center justify-center p-2 bg-green-50 text-green-600 hover:bg-green-500 hover:text-white rounded-lg transition-colors border border-green-100" title="Connect on WhatsApp"><MessageCircle size={18}/></a>
                              {booking.status !== 'Completed' && (
                                <button onClick={() => markAsCompleted(booking.id, booking.collectionName)} className="flex items-center justify-center p-2 bg-emerald-50 text-emerald-600 hover:bg-emerald-500 hover:text-white rounded-lg transition-colors border border-emerald-100" title="Complete Process & Assign App No"><CheckCircle size={18}/></button>
                              )}
                              <button onClick={() => deleteBooking(booking.id, booking.collectionName)} className="flex items-center justify-center p-2 bg-red-50 text-red-600 hover:bg-red-500 hover:text-white rounded-lg transition-colors border border-red-100" title="Remove Record"><Trash2 size={18}/></button>
                            </div>
                          </td>
                        </tr>
                      ))
        ...
        {activeTab === 'camps' && (
          <div className="animate-in fade-in slide-in-from-right-4 duration-500">
            <header className="mb-10">
              <h1 className="text-3xl font-extrabold text-gray-900">B2B Camp Inquiries</h1>
              <p className="text-gray-500 mt-1">Manage bulk registration requests from educational institutes</p>
            </header>

            <div className="grid md:grid-cols-3 gap-4 mb-8">
              <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4"><div className="bg-indigo-100 p-3 rounded-xl text-indigo-600"><Building size={24}/></div><div><p className="text-xs text-gray-500 font-bold uppercase">Total Leads</p><p className="text-2xl font-black text-gray-900">{campRequests.length}</p></div></div>
              <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4"><div className="bg-red-100 p-3 rounded-xl text-red-600"><Clock size={24}/></div><div><p className="text-xs text-gray-500 font-bold uppercase">Pending Response</p><p className="text-2xl font-black text-gray-900">{campRequests.filter(c => c.status === 'New Request').length}</p></div></div>
              <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4"><div className="bg-emerald-100 p-3 rounded-xl text-emerald-600"><CheckCircle size={24}/></div><div><p className="text-xs text-gray-500 font-bold uppercase">In Progress / Completed</p><p className="text-2xl font-black text-gray-900">{campRequests.filter(c => c.status !== 'New Request').length}</p></div></div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-gray-50 text-gray-500 text-sm uppercase tracking-wider border-b border-gray-100">
                      <th className="p-4 font-semibold">Institute Details</th>
                      <th className="p-4 font-semibold">Contact Executive</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {campRequests.length === 0 ? (
                      <tr><td colSpan="5" className="p-8 text-center text-gray-500">No B2B camp requests received yet.</td></tr>
                    ) : (
                      campRequests.map((camp) => (
                        <tr key={camp.id} className="hover:bg-gray-50 transition-colors">
                          <td className="p-4 align-top">
                            <p className="font-bold text-indigo-900 text-lg">{camp.instituteName}</p>
                            <div className="text-xs text-gray-500 mt-1 flex items-start gap-1"><MapPin size={14} className="mt-0.5 flex-shrink-0 text-gray-400"/> <span className="max-w-[200px] break-words">{camp.address}</span></div>
                            <p className="text-[10px] text-gray-400 border-t border-gray-100 pt-1 mt-2">Inquiry Date: {formatTime(camp.timestamp)}</p>
                          </td>
                          <td className="p-4 align-top">
                            <p className="font-bold text-gray-900">{camp.contactPerson}</p>
                            <p className="text-xs text-gray-500 mb-1">{camp.designation || 'Representative'}</p>
                            <p className="text-sm text-gray-600 font-medium">{camp.mobile}</p>
                            {camp.email && <p className="text-xs text-blue-600">{camp.email}</p>}
                          </td>
                          <td className="p-4 align-top"><span className="bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-sm font-bold border border-blue-200">{camp.studentCount} Forms</span></td>
                          <td className="p-4 align-top">
                            <select 
                              value={camp.status || 'New Request'}
                              onChange={(e) => updateCampStatus(camp.id, e.target.value)}
                              className={`text-sm font-bold rounded-lg px-2 py-1 outline-none border transition-colors cursor-pointer
                                ${camp.status === 'New Request' ? 'bg-red-50 text-red-700 border-red-200' : 
                                  camp.status === 'Contacted' ? 'bg-amber-50 text-amber-700 border-amber-200' : 
                                  'bg-emerald-50 text-emerald-700 border-emerald-200'}`
                              }
                            >
                              <option value="New Request">New Lead</option>
                              <option value="Contacted">Contacted</option>
                              <option value="Scheduled">Scheduled</option>
                              <option value="Completed">Camp Executed</option>
                            </select>
                          </td>
                          <td className="p-4 align-top">
                            <div className="flex items-center justify-end gap-2">
                              <a href={`https://wa.me/91${camp.mobile}?text=Hello ${camp.contactPerson}, reaching out from EduFill. We received your request to host a form-filling camp...`} target="_blank" rel="noreferrer" className="flex items-center justify-center p-2 bg-green-50 text-green-600 hover:bg-green-500 hover:text-white rounded-lg transition-colors border border-green-100" title="Contact via WhatsApp"><MessageCircle size={18}/></a>
                              <button onClick={() => deleteCampRequest(camp.id)} className="flex items-center justify-center p-2 bg-red-50 text-red-600 hover:bg-red-500 hover:text-white rounded-lg transition-colors border border-red-100" title="Delete Lead Record"><Trash2 size={18}/></button>
                            </div>
                          </td>
                        </tr>
                      ))
        ...
        {/* TAB 3: LIVE CONTROLLER VIEW */}
        {activeTab === 'liveController' && (
          <div className="animate-in fade-in slide-in-from-right-4 duration-500">
            <header className="mb-10">
              <h1 className="text-3xl font-extrabold text-gray-900">Platform Form Controls</h1>
              <p className="text-gray-500 mt-1">Manage public visibility of examination forms</p>
            </header>

            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 max-w-2xl">
              <div className="flex items-center gap-3 mb-2">
                <div className="p-3 bg-blue-50 text-blue-600 rounded-xl"><Power size={24} /></div>
                <h3 className="text-2xl font-bold text-gray-900">Real-time Form Toggles</h3>
              </div>
              <p className="text-gray-500 mb-8 pb-6 border-b border-gray-100">
                Toggle these switches to instantly show or hide active exams on the EduFill homepage without requiring a deployment refresh.
              </p>
              ...
              <div className="space-y-5">
                {['neet', 'jee', 'cuet'].map((examKey) => (
                  <div key={examKey} className="flex items-center justify-between p-5 bg-gray-50 hover:bg-gray-100 transition-colors rounded-xl border border-gray-200">
                    <div>
                      <p className="font-bold text-lg text-gray-800 uppercase tracking-wide">{examKey === 'neet' ? 'NEET UG 2026' : examKey === 'jee' ? 'JEE Main / Adv' : 'CUET UG'}</p>
                      <p className={`text-sm font-medium mt-1 ${liveExams[examKey] ? 'text-emerald-600' : 'text-gray-400'}`}>Current Status: {liveExams[examKey] ? 'Active & Visible' : 'Hidden from Users'}</p>
                    </div>
