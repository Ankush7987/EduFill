...
import { Clock, ChevronRight, ChevronLeft, CheckCircle, XCircle, AlertCircle, Award, BarChart3, Target, PlayCircle, Trophy, Download, FileText, Loader2, Key, Eye, BookOpen, PanelRightClose, PanelRightOpen, Share2, Languages, FolderOpen, ArrowLeft, ArrowRight, GraduationCap, ShieldCheck, X } from 'lucide-react';
...
export default function LiveTestPage() {
  ...
  // 🌟 NEW FIX: Lazy loading state for heavy Markdown questions to prevent thread freezing 🌟
  ...
  // 🌟 THE MASTER PERFORMANCE FIX: Progressive Loading 🌟
  const submitTest = useCallback(() => {
    setIsSubmitting(true);
    setShowDetailedResult(false);
    
    // Step 1: Let the "Analyzing Responses" loader paint smoothly on screen
    setTimeout(() => {
        calculateResult();
        
        // Step 2: Show result scores quickly without blocking thread
        setTimeout(() => {
            setIsSubmitting(false);
            setScreenState('result');
            window.scrollTo({ top: 0, behavior: 'smooth' });
            
            // Step 3: Render heavy 180 Math/Markdown questions AFTER transition completes
            setTimeout(() => {
  ...
  const renderScreen = () => {
    ...
    // ==========================================
    // 🌟 1. PROFESSIONAL LIBRARY VIEW
    // ==========================================
    if (screenState === 'library') {
      return (
        <div className="min-h-screen bg-[#F8FAFC] font-sans selection:bg-indigo-500/30">
          <div className="bg-[#0B1121] relative overflow-hidden">
            <div className="absolute top-[-20%] left-[-10%] w-[50%] h-[100%] bg-indigo-600/20 blur-[120px] pointer-events-none"></div>
            <div className="absolute bottom-[-20%] right-[-10%] w-[40%] h-[80%] bg-emerald-600/10 blur-[100px] pointer-events-none"></div>
            
            <div className="max-w-7xl mx-auto px-4 md:px-8 pt-8 pb-16 relative z-10">
              <div className="flex justify-between items-center mb-10">
                ...
                <button onClick={() => navigate('/')} className="text-gray-400 hover:text-white flex items-center gap-1.5 text-sm font-bold transition-colors border border-gray-700 bg-white/5 hover:bg-white/10 px-4 py-2 rounded-full">
                  <ArrowLeft size={16}/> Home
                </button>
              </div>

              <div className="max-w-2xl">
                <div className="inline-flex items-center gap-2 bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full mb-4 shadow-sm">
                   <span className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse"></span> 
                   Live CBT Engine
                </div>
                <h1 className="text-4xl md:text-5xl font-black text-white leading-tight mb-4">
                  {selectedYear ? `${selectedExamCategory !== 'All' ? selectedExamCategory : ''} Mock Tests of ${selectedYear}` : 'Ultimate Mock Test Library.'}
                </h1>
                <p className="text-gray-400 text-base md:text-lg font-medium">Practice official PYQs on India's most advanced Exam Simulation Engine.</p>
              </div>
            </div>
          </div>

          <div className="max-w-7xl mx-auto px-4 md:px-8 py-10 -mt-10 relative z-20">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8 bg-white p-4 rounded-[2rem] shadow-sm border border-gray-100">
              {!selectedYear && (
                 <div className="flex overflow-x-auto gap-2 custom-scrollbar w-full md:w-auto pb-2 md:pb-0">
                     {examCategoriesList.map(cat => (
                         <button 
                             key={cat}
                             onClick={() => setSelectedExamCategory(cat)}
                             className={`whitespace-nowrap px-5 py-2.5 rounded-2xl font-black text-sm transition-all ${selectedExamCategory === cat ? 'bg-gray-900 text-white shadow-md' : 'bg-gray-50 text-gray-500 border border-gray-100 hover:bg-gray-100 hover:text-gray-800'}`}
                         >
                             {cat}
                         </button>
                     ))}
              ...
              {selectedYear && (
                  <button onClick={() => setSelectedYear(null)} className="w-full md:w-auto bg-gray-900 text-white hover:bg-black px-6 py-3 rounded-2xl font-black flex items-center justify-center gap-2 transition-transform active:scale-95 shadow-md text-sm">
            ...
            {isLoadingDB ? (
              ...
              <div className="text-center py-20 bg-white rounded-[2rem] border border-gray-100 shadow-sm">
                ...
                <h3 className="text-xl font-black text-gray-900">No Mock Tests Available</h3>
                <p className="text-gray-500 mt-2 font-medium">New previous year papers will be uploaded soon by the Admin.</p>
              </div>
            ) : (
              <>
                {!selectedYear && (
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6 animate-in fade-in zoom-in-95 duration-300">
                        {yearsList.map(year => (
                            <button 
                                key={year} 
                                onClick={() => setSelectedYear(year)}
                                className="bg-white rounded-[2rem] p-6 shadow-sm border-2 border-transparent hover:border-indigo-100 hover:shadow-xl hover:-translate-y-1 transition-all flex flex-col items-center justify-center gap-4 group text-left relative overflow-hidden"
                            >
                                <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-indigo-50 to-blue-50 opacity-50 rounded-bl-full -z-10 group-hover:scale-150 transition-transform duration-500"></div>
                                <div className="w-16 h-16 rounded-2xl bg-indigo-50/50 text-indigo-600 flex items-center justify-center group-hover:bg-indigo-600 group-hover:text-white transition-colors duration-300 border border-indigo-100">
                                    <FolderOpen size={32} />
                                </div>
                                <div className="text-center">
                                    <h3 className="text-2xl font-black text-gray-900 group-hover:text-indigo-600 transition-colors">{year}</h3>
                                    <p className="text-xs font-bold text-gray-500 mt-1 uppercase tracking-widest">{groupedPapers[year].length} Papers</p>
                                </div>
                            </button>
                        ))}
                    </div>
                ...
                {selectedYear && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-in slide-in-from-right-8 duration-300">
                        {groupedPapers[selectedYear].map((paper) => {
                          const testUrl = `/mock-test/${createSlug(paper.title)}`;
                          return (
                            <div key={paper.id} className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100 hover:shadow-xl hover:border-indigo-200 transition-all flex flex-col group relative overflow-hidden">
                                <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-indigo-50 to-emerald-50 opacity-50 rounded-bl-[100px] -z-10"></div>
                                <div className="flex justify-between items-start mb-4">
                                  <div className="bg-gray-900 text-white text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full">
                                      {paper.examName || 'Mock Test'}
                                  </div>
                                  <span className="text-gray-400 font-black text-sm bg-gray-50 border border-gray-100 px-3 py-1 rounded-lg">{paper.year || 'N/A'}</span>
                                </div>
                                <Link to={testUrl} onClick={(e) => { e.preventDefault(); viewTestDetails(paper); }} className="text-xl font-black text-gray-900 mb-6 group-hover:text-indigo-600 transition-colors line-clamp-2 leading-tight">
                                    {paper.title}
                                </Link>
                                <div className="grid grid-cols-2 gap-3 mb-6 mt-auto">
                                  <div className="flex flex-col items-center justify-center bg-gray-50 p-3 rounded-2xl border border-gray-100">
                                    <FileText size={18} className="text-indigo-500 mb-1"/>
                                    <span className="text-xs font-black text-gray-800">{paper.totalQuestions || paper.questions?.length || 0} Qs</span>
                                  </div>
                                  <div className="flex flex-col items-center justify-center bg-gray-50 p-3 rounded-2xl border border-gray-100">
                                    <Clock size={18} className="text-amber-500 mb-1"/>
                                    <span className="text-xs font-black text-gray-800">{formatTime((paper.durationMins || 180) * 60)}</span>
                                  </div>
                                </div>
                                <div className="mt-auto space-y-3">
                                  <div className="grid grid-cols-2 gap-3">
                                      <a href={paper.paperPdfUrl || '#'} target="_blank" rel="noreferrer" onClick={(e) => { if(!paper.paperPdfUrl || paper.paperPdfUrl === '#') { e.preventDefault(); alert('PDF not available'); } }} className="flex justify-center items-center gap-1.5 bg-white hover:bg-gray-50 border-2 border-gray-100 text-gray-600 hover:text-gray-900 font-bold py-3 rounded-xl transition-colors text-xs active:scale-95">
                                  ...
                                  <Link to={testUrl} onClick={(e) => { e.preventDefault(); viewTestDetails(paper); }} className="w-full flex justify-center items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-black py-4 rounded-xl shadow-lg transition-transform active:scale-95 text-sm">
    ...
    if (screenState === 'test-landing') {
        ...
        return (
          <div className="min-h-screen bg-[#F8FAFC] py-6 md:py-12 px-4 sm:px-6 lg:px-8 font-sans flex items-center justify-center relative overflow-hidden">
            <div className="absolute top-[-20%] left-[-10%] w-[50%] h-[100%] bg-indigo-600/10 blur-[120px] pointer-events-none"></div>
            <div className="absolute bottom-[-20%] right-[-10%] w-[40%] h-[80%] bg-emerald-600/10 blur-[100px] pointer-events-none"></div>

            <div className="max-w-3xl w-full bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-gray-100 relative z-10 animate-in fade-in zoom-in-95 duration-500">
              <div className="bg-[#0B1121] p-8 md:p-12 text-white relative overflow-hidden">
                 ...
                 <div className="flex justify-between items-start mb-8 relative z-10">
                   <div className="bg-white/10 px-4 py-2 rounded-full border border-white/20 flex items-center gap-2">
                     ...
                     <span className="text-xs font-black uppercase tracking-widest text-emerald-100">Official Exam Simulation</span>
                   </div>
                   <button onClick={resetEngine} className="text-gray-400 hover:text-white transition-colors bg-white/5 hover:bg-white/10 p-2 rounded-full"><X size={20}/></button>
                 </div>
                 <h1 className="text-3xl md:text-5xl font-black mb-4 relative z-10 leading-tight">{activePaper?.title}</h1>
                 <div className="flex flex-wrap gap-3 relative z-10">
                    <span className="bg-white/5 px-4 py-1.5 rounded-lg font-bold text-xs border border-white/10 text-gray-300">{activePaper?.examName}</span>
                    <span className="bg-white/5 px-4 py-1.5 rounded-lg font-bold text-xs border border-white/10 text-gray-300">{activePaper?.year} Series</span>
                 </div>
              </div>
              <div className="p-6 md:p-10 bg-white">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
                   <div className="bg-gray-50 p-4 md:p-5 rounded-2xl border border-gray-100 text-center">
                      <FileText className="mx-auto text-indigo-500 mb-2" size={24}/>
                      <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest mb-0.5">Evaluate Qs</p>
                      <p className="text-xl md:text-2xl font-black text-gray-900">{evaluableQuestionsCount}</p>
                   </div>
                   <div className="bg-gray-50 p-4 md:p-5 rounded-2xl border border-gray-100 text-center">
                      <Clock className="mx-auto text-amber-500 mb-2" size={24}/>
                      <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest mb-0.5">Duration</p>
                      <p className="text-xl md:text-2xl font-black text-gray-900">{activePaper?.durationMins} M</p>
                   </div>
                   <div className="bg-gray-50 p-4 md:p-5 rounded-2xl border border-gray-100 text-center">
                      <Award className="mx-auto text-emerald-500 mb-2" size={24}/>
                      <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest mb-0.5">Max Marks</p>
                      <p className="text-xl md:text-2xl font-black text-gray-900">{totalMarks}</p>
                   </div>
                   <div className="bg-gray-50 p-4 md:p-5 rounded-2xl border border-gray-100 text-center">
                      <Target className="mx-auto text-red-500 mb-2" size={24}/>
                      <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest mb-0.5">Negative</p>
                      <p className="text-xl md:text-2xl font-black text-gray-900">-1</p>
                   </div>
                </div>
                <div className="flex flex-col sm:flex-row gap-4">
                   <button onClick={resetEngine} className="flex-1 bg-white hover:bg-gray-50 border-2 border-gray-200 text-gray-600 font-bold py-4 px-6 rounded-xl transition-colors flex items-center justify-center gap-2 text-sm active:scale-95">
                      <ArrowLeft size={18}/> Return to Library
                   </button>
                   <button onClick={goToInstructions} className="flex-[2] bg-gray-900 hover:bg-black text-white font-black py-4 px-8 rounded-xl shadow-xl transition-transform active:scale-95 flex items-center justify-center gap-2 text-base">
                      Read Instructions <ArrowRight size={18}/>
                   </button>
                </div>
              </div>
            </div>
    ...
    if (screenState === 'instructions') {
      return (
        <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center p-4 font-sans relative overflow-hidden">
          ...
          <div className="bg-white p-8 md:p-12 rounded-[2rem] shadow-2xl max-w-2xl w-full flex flex-col animate-in zoom-in-95 duration-300 relative z-10 border border-gray-100">
            <div className="flex justify-between items-center mb-8 pb-4 border-b border-gray-100">
              <div>
                <h1 className="text-2xl md:text-3xl font-black text-gray-900 flex items-center gap-3">
                  <AlertCircle className="text-indigo-600"/> Read Carefully
                </h1>
                <p className="text-gray-500 font-bold mt-2 text-sm">{activePaper?.title}</p>
              </div>
            </div>
            <div className="space-y-6 mb-10 bg-gray-50 p-6 rounded-2xl border border-gray-100">
              <div className="flex items-start gap-4">
                <div className="w-10 h-10 rounded-xl bg-white border border-gray-200 flex items-center justify-center text-gray-900 font-black shrink-0 shadow-sm">1</div>
                <div>
                  <h4 className="font-black text-gray-900 text-lg mb-1">Test Pattern</h4>
                  <p className="text-gray-500 text-sm font-medium leading-relaxed">This test contains {questions.length} questions. Maximum {evaluableQuestionsCount} questions will be evaluated. Total duration is {formatTime((activePaper?.durationMins || 180) * 60)}. Timer cannot be paused once started.</p>
                </div>
              </div>
              <div className="flex items-start gap-4">
                <div className="w-10 h-10 rounded-xl bg-white border border-gray-200 flex items-center justify-center text-gray-900 font-black shrink-0 shadow-sm">2</div>
                <div>
                  <h4 className="font-black text-gray-900 text-lg mb-2">Marking Scheme</h4>
                  <ul className="text-gray-500 text-sm font-bold space-y-2">
                    <li className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-emerald-500"></span> +4 marks for correct answer.</li>
                    <li className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-red-500"></span> -1 mark for incorrect answer.</li>
                    <li className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-gray-300"></span> 0 marks for unattempted.</li>
                  </ul>
                </div>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row gap-4">
              <button onClick={() => setScreenState('test-landing')} className="flex-1 py-4 bg-white border-2 border-gray-200 hover:bg-gray-50 text-gray-600 font-bold rounded-xl transition-colors active:scale-95">Decline</button>
              <button onClick={beginExam} className="flex-[2] bg-indigo-600 hover:bg-indigo-700 text-white font-black py-4 rounded-xl shadow-lg transition-transform active:scale-95 flex justify-center items-center gap-2 text-lg">
                <PlayCircle size={20}/> Begin Exam Now
              </button>
            </div>
          </div>
        </div>
    ...
    if (screenState === 'exam') {
      ...
      const handleSubjectTabClick = (subj) => {
        ...
        if (firstQuestionIndex !== -1) handleNavigate(firstQuestionIndex);
      };
        ...
        <div className="fixed inset-0 z-[100] bg-[#f8f9fa] flex flex-col font-sans overflow-hidden animate-in fade-in duration-300">
          ...
          <header className="bg-white border-b border-gray-200 p-3 md:px-6 md:py-3 flex justify-between items-center shrink-0 shadow-sm z-10">
            ...
            <div className="flex items-center gap-2 md:gap-3 shrink-0">
              <button onClick={() => setLanguage(prev => prev === 'en' ? 'hi' : 'en')} className="hidden sm:flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 bg-gray-50 hover:bg-indigo-50 border border-gray-200 text-gray-700 rounded-xl shadow-sm font-bold text-sm transition-all active:scale-95">
                <Languages size={18}/> <span>{language === 'en' ? 'English' : 'हिन्दी'}</span>
              </button>
          ...
          <div className="flex-1 flex w-full h-full min-h-0 overflow-hidden relative">
            <div className={`flex-1 bg-white flex flex-col min-h-0 transition-all duration-300 shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-0 ${isPaletteOpen ? 'lg:mr-[320px] xl:mr-[380px]' : ''}`}>
              <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-10">
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-100">
                  ...
                  <button onClick={() => setLanguage(prev => prev === 'en' ? 'hi' : 'en')} className="sm:hidden flex items-center gap-1 px-2 py-1 bg-indigo-50 border border-indigo-100 text-indigo-700 rounded-lg font-bold text-[10px]">
                    <Languages size={12}/> {language === 'en' ? 'EN' : 'HI'}
                  </button>
                  <div className="hidden sm:flex gap-2">
                    <span className="bg-emerald-50 text-emerald-700 font-bold px-3 py-1.5 rounded-lg text-xs border border-emerald-100 flex items-center gap-1.5">+4 Marks</span>
                    <span className="bg-red-50 text-red-700 font-bold px-3 py-1.5 rounded-lg text-xs border border-red-100 flex items-center gap-1.5">-1 Negative</span>
                  </div>
                </div>
              ...
              <div className="bg-white border-t border-gray-200 p-3 md:p-4 shrink-0 shadow-[0_-4px_20px_rgba(0,0,0,0.03)] z-10 flex flex-wrap justify-between items-center gap-2 md:gap-3">
                <div className="flex gap-2 w-full md:w-auto order-2 md:order-1">
                  <button onClick={markForReview} className="flex-1 bg-white text-gray-700 hover:text-amber-600 border border-gray-300 font-bold py-2 md:py-3 px-2 md:px-5 rounded-xl text-xs md:text-sm transition-all shadow-sm flex items-center justify-center gap-1.5">
            ...
            <div className={`fixed inset-y-0 right-0 lg:absolute lg:inset-y-auto lg:top-0 lg:bottom-0 w-[85vw] sm:w-[320px] xl:w-[380px] bg-white border-l border-gray-200 flex flex-col z-50 lg:z-20 transition-transform duration-300 shadow-2xl lg:shadow-none ${isPaletteOpen ? 'translate-x-0' : 'translate-x-full'}`}>
              ...
              <div className="p-3 border-b border-gray-100 shrink-0 grid grid-cols-2 gap-y-2 gap-x-1 text-[10px] sm:text-xs font-bold text-gray-600">
                <div className="flex items-center gap-1.5"><div className="w-3.5 h-3.5 rounded bg-emerald-500 border border-emerald-600"></div> Answered</div>
                ...
                <div className="flex items-center gap-1.5"><div className="w-3.5 h-3.5 rounded bg-amber-400 border border-amber-500"></div> Marked</div>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-3 bg-gray-50/50">
                <div className="grid grid-cols-5 sm:grid-cols-6 lg:grid-cols-5 gap-2">
                  {questions.map((q, idx) => {
                    ...
                    if (status === 'answered') bgColor = "bg-emerald-500 text-white border-emerald-600 shadow-sm";
                    else if (status === 'marked') bgColor = "bg-amber-400 text-white border-amber-500 shadow-sm";
              ...
              <div className="p-3 border-t border-gray-200 bg-white shrink-0">
                <button 
                  ...
                  className="w-full bg-gray-900 hover:bg-black text-white font-black py-3.5 rounded-xl shadow-lg transition-transform active:scale-95 text-sm sm:text-base flex justify-center items-center gap-2"
                >
    ...
    if (screenState === 'result') {
      ...
      return (
        <div className="min-h-screen bg-[#F8FAFC] py-6 px-4 font-sans relative overflow-hidden">
          <div className="absolute top-[-20%] left-[-10%] w-[50%] h-[100%] bg-indigo-600/10 blur-[120px] pointer-events-none"></div>

          <div className="max-w-5xl mx-auto bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-gray-100 relative z-10 animate-in slide-in-from-bottom-8 duration-500">
            
            <div className="bg-[#0B1121] p-8 md:p-12 text-white text-center relative overflow-hidden">
              <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/20 blur-3xl rounded-full pointer-events-none"></div>
              
              <button onClick={resetEngine} className="absolute top-6 right-6 bg-white/10 hover:bg-white/20 p-2 rounded-full transition-colors z-10"><XCircle size={24}/></button>
              
              <Trophy size={64} className="mx-auto mb-6 text-yellow-400 drop-shadow-lg" />
              <p className="text-xs uppercase tracking-widest text-gray-400 font-black mb-2">Performance Report</p>
              <h2 className="text-2xl md:text-4xl font-black mb-3 text-white leading-tight">{activePaper?.title}</h2>
              <p className={`text-lg font-bold ${remarkColor}`}>{remark}</p>
              
              <button onClick={handleShare} className="mt-8 mx-auto flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white font-black py-3 px-8 rounded-xl shadow-lg transition-transform hover:scale-105 active:scale-95 z-10 relative text-sm md:text-base">
            ...
            <div className="p-4 md:p-10 bg-white">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-10">
                <div className="bg-gray-50 p-4 md:p-6 rounded-2xl border border-gray-100 text-center relative overflow-hidden group hover:border-indigo-300 transition-colors">
                  <p className="text-gray-400 text-[10px] md:text-xs font-black uppercase mb-1 tracking-widest">Total Score</p>
                  <p className="text-3xl md:text-4xl font-black text-indigo-600">{scoreData.marks}</p>
                </div>
                <div className="bg-gray-50 p-4 md:p-6 rounded-2xl border border-gray-100 text-center relative overflow-hidden group hover:border-emerald-300 transition-colors">
                  <p className="text-gray-400 text-[10px] md:text-xs font-black uppercase mb-1 tracking-widest">Accuracy</p>
                  <p className="text-3xl md:text-4xl font-black text-emerald-600">{accuracy}%</p>
                </div>
                <div className="bg-gray-50 p-4 md:p-6 rounded-2xl border border-gray-100 text-center relative overflow-hidden group hover:border-red-300 transition-colors">
                  <p className="text-gray-400 text-[10px] md:text-xs font-black uppercase mb-1 tracking-widest">Incorrect</p>
                  <p className="text-3xl md:text-4xl font-black text-red-500">{scoreData.incorrect}</p>
                </div>
                <div className="bg-gray-50 p-4 md:p-6 rounded-2xl border border-gray-100 text-center relative overflow-hidden group hover:border-gray-300 transition-colors">
                  <p className="text-gray-400 text-[10px] md:text-xs font-black uppercase mb-1 tracking-widest">Skipped</p>
                  <p className="text-3xl md:text-4xl font-black text-gray-500">{scoreData.unattempted}</p>
                </div>
              </div>
              
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-8 gap-4 border-b border-gray-100 pb-6">
                ...
                <div className="flex items-center gap-3">
                  <button 
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-gray-50 hover:bg-gray-100 border border-gray-200 text-gray-700 rounded-xl font-bold shadow-sm transition-colors text-xs md:text-sm"
                  >
                    <Languages size={16}/> 
                    <span className="hidden sm:inline">{language === 'en' ? 'Switch to Hindi' : 'Switch to English'}</span>
                  </button>

                  <a href={activePaper?.answerKeyPdfUrl || '#'} target="_blank" rel="noreferrer" onClick={(e) => { if(!activePaper?.answerKeyPdfUrl || activePaper?.answerKeyPdfUrl === '#') { e.preventDefault(); alert('Answer Key not available'); } }} className="flex items-center justify-center gap-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 px-4 py-2 rounded-xl font-bold shadow-sm transition-colors text-xs md:text-sm">
              ...
              <div className="space-y-6 md:space-y-8">
                {!showDetailedResult ? (
                  <div className="flex flex-col items-center justify-center py-16 bg-white rounded-[2rem] border border-gray-100 shadow-sm animate-in fade-in duration-300">
                    <Loader2 className="w-10 h-10 animate-spin text-indigo-600 mb-4" />
                    <p className="text-gray-500 font-black tracking-widest uppercase text-xs">Generating Question Analysis...</p>
                  </div>
                ) : (
                  questions.map((q, idx) => {
                    ...
                    return (
                      <div key={q.id || idx} className={`bg-white p-5 md:p-8 rounded-3xl border-2 shadow-sm animate-in slide-in-from-bottom-4 duration-500 ${isCorrect ? 'border-emerald-200' : isUnattempted ? 'border-gray-200' : 'border-red-200'}`}>
                        <div className="flex flex-col sm:flex-row justify-between items-start mb-6 gap-4">
                          <div className="flex gap-3 md:gap-4 w-full">
                            <div className={`w-8 h-8 md:w-10 md:h-10 shrink-0 rounded-xl flex items-center justify-center font-black text-white text-sm md:text-base ${isCorrect ? 'bg-emerald-500' : isUnattempted ? 'bg-gray-400' : 'bg-red-500'}`}>
                              {idx + 1}
                            </div>
                          ...
                          <span className="hidden sm:block shrink-0 text-[10px] font-black text-indigo-700 uppercase tracking-widest bg-indigo-50 px-3 py-1.5 rounded-lg border border-indigo-100">{getSafeSubject(q, idx, questions.length)}</span>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6 pl-0 sm:pl-12 md:pl-14">
                          {getLocalizedOptions(q.options).map((opt, oIdx) => (
                            <div key={oIdx} className={`p-3 md:p-4 rounded-xl text-xs md:text-sm flex justify-between items-center font-bold transition-all border-2 overflow-x-auto ${oIdx === q.correctOptionIndex ? "bg-emerald-50 border-emerald-500 text-emerald-800" : (userAns === oIdx && !isCorrect) ? "bg-red-50 border-red-500 text-red-800" : "border-gray-100 bg-gray-50 text-gray-600"}`}>
                              ...
                              {oIdx === q.correctOptionIndex && <CheckCircle size={18} className="text-emerald-600 drop-shadow-sm shrink-0 ml-2"/>}
                              {(userAns === oIdx && !isCorrect) && <XCircle size={18} className="text-red-600 drop-shadow-sm shrink-0 ml-2"/>}
                            </div>
                          ))}
                        </div>

                        <div className="bg-gray-50 p-4 md:p-6 rounded-2xl border border-gray-100 text-xs md:text-sm font-medium text-gray-700 mt-6 ml-0 sm:pl-12 md:ml-14">
                          <h4 className="text-gray-900 font-black text-sm md:text-base flex items-center gap-1.5 mb-2 md:mb-3"><FileText size={16} className="text-indigo-500"/> {language === 'en' ? 'Expert Explanation' : 'विशेषज्ञ व्याख्या'}</h4> 
                          <div className="text-gray-600 leading-relaxed overflow-x-auto"><SafeMath text={getLocalizedText(q.explanation) || (language === 'en' ? "Refer to the official answer key for detailed steps." : "विस्तृत चरणों के लिए आधिकारिक उत्तर कुंजी देखें।")} /></div>
                        </div>
                      </div>
                    );
                  })
              ...
              <div className="mt-10 flex justify-center">
                 <button onClick={resetEngine} className="bg-gray-900 hover:bg-black text-white font-black py-4 px-12 rounded-xl text-base shadow-xl transition-transform hover:scale-105 active:scale-95">Return to Dashboard</button>
              </div>
            </div>
          </div>
        </div>
